...
def complete_(data): # [1pts]
    ...
    return np.asarray(labeled_complete)
...
def incomplete_(data): # [1pts]
    ...
    labeled_incomplete = []
    for i in range(len(data)):
        if np.isnan(data[i][:-1]).any() and ~np.isnan(data[i][-1]):
            labeled_incomplete.append(data[i])
    return np.asarray(labeled_incomplete)
...
def unlabeled_(data): # [1pts]
    ...
    return np.asarray(unlabeled_complete)
...
class CleanData(object):
    ...
    def __call__(self, incomplete_points,  complete_points, K, **kwargs): # [7pts]
        ...
        """
        #raise NotImplementedError
        cols = incomplete_points.shape[1]-1
        labels_ip = incomplete_points[:,cols:]
        labels_cp = complete_points[:,cols:]
        labels = np.concatenate((labels_ip, labels_cp)).reshape(-1)
