...
class LogisticRegression(object):
    ...
    def predict_probs(self, x_aug: np.ndarray, theta: np.ndarray) -> np.ndarray: # [5pts]
        ...
        """
        #raise NotImplementedError
        sigmoid = self.sigmoid(x_aug @ theta)
    ...
    def loss(self, y: np.ndarray, h_x: np.ndarray) -> float: # [3pts]
        ...
        """
        #raise NotImplementedError
        # -[y * log(h_x)) + (1-y)*log(1 - h_x)]/N

        p0 = y*np.log(h_x)
        p1 = (np.ones_like(y) - y) * np.log(np.ones_like(h_x) - h_x)
        cross_entropy = -1*(p0 + p1)
        bce = np.sum(cross_entropy)/len(y)
    ...
    def accuracy(self, y: np.ndarray, y_hat: np.ndarray) -> float: # [2pts]
        ...
        """
        #raise NotImplementedError
        # Number of times y = y_hat
        number_of_matched_preds = np.sum(y == y_hat)
        accuracy = number_of_matched_preds/len(y)
    ...
    def fit(
        ...
        x_aug = self.bias_augment(x = x_train)
        ...
        for i in range(epochs):
            ...
            if np.mod(i, 100) == 0:
                self.update_evaluation_lists(x_train, y_train, x_val, y_val, theta, epoch = i)
