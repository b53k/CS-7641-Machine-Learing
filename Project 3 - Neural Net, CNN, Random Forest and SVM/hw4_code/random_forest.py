...
class RandomForest(object):
    ...
    def _bootstrapping(self, num_training, num_features, random_seed = None):
        ...
        random_features = np.int(np.floor(num_features * self.max_features))
        col_idx = np.random.choice(num_features, random_features, replace = False)
        ##################################################################################
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_estimators):
            bootstrap_row_idx = self.bootstraps_row_indices[i]                  # get an instance of training sample
            bootstrap_col_idx = self.feature_indices[i]                         # get column indices from bootstrap dataset
            X_new = X[bootstrap_row_idx, :]                                     # select all bootstrapped rows
            X_new = X_new[:, bootstrap_col_idx]                                 # select all bootstrapped columns
            y_new = y[bootstrap_row_idx]                                        # labels corresponding to selected rows
            self.decision_trees[i] = self.decision_trees[i].fit(X_new, y_new)   # fit decision tree for bootstrapped dataset
    ...
    def plot_feature_importance(self, data_train):
        ...
        y_train = data_train['ediblilty'].to_numpy()
        #feature_names = [name for name in X_train.columns]

        number_of_plots = 10
        row = np.ceil(number_of_plots/2)
        col = number_of_plots - row

        self.n_estimators, self.max_depth, self.max_features = self.select_hyperparameters()
        self.fit(X_train.to_numpy(), y_train)
        k = 0

        for i in range(number_of_plots):
            importance = self.decision_trees[i].feature_importances_
            importance = importance[importance != 0] # remove 0's in the importance array
            idx = np.argsort(importance)[::-1] # descending order
            x = data_train.columns[idx]
            y = importance[idx]
            ...
            if i in [0,1,4,6,8,9]:
                ...
                plt.title('Feature Importance for Extra Tree: ' + str(i), fontsize = 14)
